...
基于百川模型的全模态模型推理客户端
---------------------------------------------------
• 把 Env 观测 (image np.array, audio np.array, state vec)
  → 构造成符合百川API的多模态 prompt
• 调用百川接口
• 解析模型回复为离散动作 id
"""
...
# --- 百川 FastAPI 服务器配置 ---
FASTAPI_BASE_URL = ""  # 百川模型的FastAPI服务器地址
    ...
    def act(self, obs: dict) -> int:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                end_game = payload_end_game  # 从payload获取end_game值
    ...
    def _build_payload(self, obs: dict, current_round_data: dict) -> dict:
        ...
        try:
            ...
            if isinstance(audio, str) and audio.startswith("data:"):
                try:
                    # 解析data URL格式: data:mime_type;base64,base64_data
